...
class SystemLocaleServiceTest {
    ...
    void testFindActiveByCodeSuccessfully() {
        when(systemLocaleRepository.findActiveByCode(LOCALE_CODE)).thenReturn(Optional.of(systemLocale));
    ...
    void testFindActiveByCodeNotFound() {
        ...
        assertThrows(EntityNotFoundException.class,
                () -> systemLocaleService.findActiveByCode(LOCALE_CODE));

        verify(systemLocaleRepository).findActiveByCode(LOCALE_CODE);
    }

    @Test
    @DisplayName("Should throw EntityNotFoundException with correct message when system locale not found")
    void testFindActiveByCodeNotFoundMessage() {
        ...
        assert exception.getMessage().equals("System locale not found") : "Exception message should match";
        verify(systemLocaleRepository).findActiveByCode(LOCALE_CODE);
    }
}
