...
class RateLimitServiceTest {
  ...
  @Test
  @DisplayName(
      "Should return different bucket instances for different paths with same IP when using default rule")
  ...
  void testResolveBucketWithNullRules() {
    ...
    assertNotNull(bucket);
    assert bucket.getAvailableTokens() == CAPACITY
        : "Bucket should have default capacity when rules is null";
  }
  ...
  void testResolveBucketWithEmptyRules() {
    ...
    assertNotNull(bucket);
    assert bucket.getAvailableTokens() == CAPACITY
        : "Bucket should have default capacity when rules is empty";
  }
  ...
  void testResolveBucketThrowsExceptionWhenDefaultRuleIsNull() {
    ...
    IllegalStateException exception =
        assertThrows(IllegalStateException.class, () -> rateLimitService.resolveBucket(IP, PATH));

    assert exception
            .getMessage()
            .contains("Rate limit rule not found and default rule is not configured")
  ...
  void testResolveBucketWithNullDefaultRulePath() {
    ...
    assertNotNull(bucket);
    assert bucket.getAvailableTokens() == CAPACITY
        : "Bucket should be created even when default rule path is null";
  }
  ...
  void testResolveBucketUsesDefaultRuleWhenNoMatch() {
    ...
    assertNotNull(bucket);
    assert bucket.getAvailableTokens() == CAPACITY
        : "Bucket should use default rule when no rules match";
  }

  @Test
  @DisplayName(
      "Should return different bucket instances for different paths even when using same rule")
  void testResolveBucketReturnsDifferentInstancesForDifferentPathsWithSameRule() {
    ...
    assert bucket1 != bucket2
        : "Should return different bucket instances for different paths even with same rule";
