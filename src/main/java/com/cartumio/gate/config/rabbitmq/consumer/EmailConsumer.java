...
public class EmailConsumer {
  ...
  public void consumeEmail(Email email) {
    log.info(
        "Consuming email from queue | subject={}, to={}",
        email.getSubject(),
        email.getTo() != null
            ? email.getTo().stream()
    ...
    try {
      ...
      log.error(
          "Email processing failed | subject={}, error={}", email.getSubject(), e.getMessage(), e);
      throw e;
    } catch (Exception e) {
      log.error(
          "Unexpected error processing email | subject={}, error={}",
          email.getSubject(),
          e.getMessage(),
          e);
