...
public class EmailService {
  ...
  public void processEmail(Email email) {
    log.info(
        "Processing email | subject={}, to={}, from={}",
        email.getSubject(),
  ...
  private void processBody(Email email) {
    ...
    log.debug(
        "Processing email body with Mustache template | dataKeys={}", email.getData().keySet());
    try {
  ...
  private EmailRequest buildEmailRequest(Email email) {
    log.debug(
        "Building email request | subject={}, recipientsCount={}",
        email.getSubject(),
        email.getTo().size());
    return new EmailRequest(
        email.getSubject(),
        new EmailUserRequest(email.getFrom()),
        email.getTo().stream().map(EmailUserRequest::new).collect(Collectors.toList()),
        email.getBody());
  }

  private void sendEmail(EmailRequest email) {
    log.info(
        "Sending email via Brevo API | subject={}, to={}",
        email.subject(),
        email.to().stream().map(EmailUserRequest::email).collect(Collectors.joining(", ")));
    try {
      brevoRestClient
          .post()
          .uri("/smtp/email")
      ...
      log.error(
          "Error sending email via Brevo API | subject={}, error={}",
          email.subject(),
          e.getMessage(),
          e);
      throw new EmailFailedException("Error sending email", e);
    }
  }
}
