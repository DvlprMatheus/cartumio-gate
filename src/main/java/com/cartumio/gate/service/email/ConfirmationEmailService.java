...
public class ConfirmationEmailService {
  ...
  public void sendConfirmationEmail(
    ...
    log.info(
        "Sending confirmation email | fullName={}, email={}, language={}",
        fullName,
        email,
        language);

    EmailTemplate emailTemplate =
        emailTemplateService.findActiveByCodeAndLanguage("confirmation-template", language);

    log.debug("Email template found | code={}, language={}", emailTemplate.getCode(), language);

    tokenService.invalidateAllNonConsumedForEmail(TokenType.EMAIL_CONFIRMATION, email);
    ...
    log.debug(
        "Token generated for confirmation email | tokenType={}, expiresAt={}",
        token.tokenType(),
        token.expiresAt());
    ...
    Email request =
        new Email()
            ...
            .addFrom(
                new EmailUser(properties.getSender().getName(), properties.getSender().getEmail()))
    ...
    log.debug(
        "Confirmation email request built | subject={}, to={}", emailTemplate.getSubject(), email);
